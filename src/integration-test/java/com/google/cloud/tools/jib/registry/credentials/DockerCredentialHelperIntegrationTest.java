...
public class DockerCredentialHelperIntegrationTest {
    ...
    @Test
    public void testRetrieve_nonexistentCredentialHelper() throws IOException, NonexistentServerUrlDockerCredentialHelperException {
        try {
            DockerCredentialHelper fakeDockerCredentialHelper = new DockerCredentialHelperFactory("").withCredentialHelperSuffix("fake-cloud-provider");
            ...
            Assert.assertEquals("The system does not have docker-credential-fake-cloud-provider CLI", ex.getMessage());
        }
    }

    @Test
    public void testRetrieve_nonexistentServerUrl() throws IOException, NonexistentDockerCredentialHelperException {
        try {
            DockerCredentialHelper fakeDockerCredentialHelper = new DockerCredentialHelperFactory("fake.server.url").withCredentialHelperSuffix("gcr");
            fakeDockerCredentialHelper.retrieve();
            ...
            Assert.assertThat(ex.getMessage(), CoreMatchers.containsString("The credential helper (docker-credential-gcr) has nothing for server URL: fake.server.url"));
        }
    }
}
