...
public class CacheReaderTest {
    ...
    public void testGetLayerFile() throws CacheMetadataCorruptedException, IOException {
        Path expectedFile = testCacheFolder.resolve("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.tar.gz");
        try (Cache cache = Cache.init(testCacheFolder)) {
            CacheReader cacheReader = new CacheReader(cache);
            Assert.assertEquals(expectedFile, cacheReader.getLayerFile(Collections.singletonList(Paths.get("some", "source", "directory"))));
    ...
    public void testGetUpToDateLayerBySourceFiles() throws URISyntaxException, IOException, CacheMetadataCorruptedException {
        ...
        try (Stream<Path> fileStream = Files.walk(testSourceFiles)) {
            fileStream.sorted(Comparator.reverseOrder()).forEach(path -> {
                try {
